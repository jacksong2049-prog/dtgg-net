...
DTGG 是一个聚焦“郑州电梯广告投放咨询”的静态获客站。当前重点是电梯轿厢内部电子显示屏广告，可播放视频或展示图片。网站目标不是泛广告平台，也不是传统广告公司官网，而是帮助访客快速获取郑州电梯广告报价区间、点位资源表和投放建议，并引导添加广告顾问微信或提交需求表单。
...
```text
public/wechat-qr.svg
```

替换方式：

1. 将朋友的微信二维码图片放到 `public` 目录，例如 `public/wechat-qr.png`。
2. 修改 `config/site.ts`：
